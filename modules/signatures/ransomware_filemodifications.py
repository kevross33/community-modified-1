...
class RansomwareFileModifications(Signature):
    ...
    def on_complete(self):
        ...
        if self.appendemailcount > 60:
            self.data.append({"appends_email" : "Appears to have appended an email address onto %s files. This is used by ransomware which requires the user to email the attacker for recovery actions/payment" % (self.appendemailcount)})

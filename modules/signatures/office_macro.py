                ...
                if "SummaryInformation" in self.results["static"]["office"]["Metadata"]:
                    ...
                    if re.search("[0-9]{1}", author) and re.search("[A-Z]{1}", author):
                        if len(author) < 6 and re.match("^[a-zA-Z0-9]{1,5}$", author):
                            self.severity = 3
                            self.weight += 2
                            self.data.append({"short_author_format" : "The file author is a short name yet contains numerical and upper case characters in it indicative of an automated document creation kit."})
                    ...
                    if re.search("[0-9]{1}", lastauthor) and re.search("[A-Z]{1}", lastauthor):
                        if len(lastauthor) < 6 and re.match("^[a-zA-Z0-9]{1,5}$", lastauthor):
                            self.severity = 3
                            self.weight += 2
                            self.data.append({"short_last_saved_format" : "The file was last saved by an author with a short name yet containing numerical and upper case characters in it indicative of an automated document creation kit."})
